...
class ChatUI:
    ...
    def display_chat_history(self):
        ...
        for message in st.session_state.visible_messages:
            ...
            # ASSISTANT MESSAGES
                ...
                # Check if this is raw HTML that should be escaped and treated as a regular message
                if ("<div style=" in content and "background-color:" in content and 
                    "Example:" in content and "Question:" in content):
                    # This is raw HTML that should be parsed and displayed properly
                    self._display_example_from_html(content)
                    
                # ENHANCED EXAMPLE & QUESTION BOX (formatted text)
                elif "*Example:" in content or "Example:" in content:
                    self._display_example_and_question(content)
                    
                # HELP BOX
                elif ("help" in content.lower() and ("understanding" in content.lower() or "explanation" in content.lower())) or content.startswith("I need help"):
                    st.markdown(
                        f"""
                ...
                # REGULAR ASSISTANT MESSAGE
                    ...
                    # Escape any raw HTML that shouldn't be rendered
    ...
    def _display_example_from_html(self, html_content):
        """Parse raw HTML content and extract example and question to display properly."""
        import re
        
        # Try to extract example text from HTML
        example_match = re.search(r'Example:</p>\s*<p[^>]*>([^<]+)</p>', html_content)
        example_text = example_match.group(1) if example_match else ""
        
        # Try to extract question text from HTML
        question_match = re.search(r'Question:</p>\s*<p[^>]*>([^<]+)</p>', html_content)
        question_text = question_match.group(1) if question_match else ""
        
        # If we couldn't parse the HTML properly, fall back to a simple display
        if not example_text and not question_text:
            # Try a broader extraction
            example_match = re.search(r'Example:(.*?)Question:', html_content, re.DOTALL)
            if example_match:
                example_text = re.sub(r'<[^>]+>', '', example_match.group(1)).strip()
    ...
    def _display_example_and_question(self, content):
        ...
        if not question_text and remaining and remaining != content:
            question_text = remaining
        
        # Use the common rendering method
        self._render_example_box(example_text, question_text)
    ...
    def display_completion_ui(self, summary_text, export_service, user_info, responses):
        ...
        if st.session_state.get("explicitly_finished", False):
            ...
            st.info("Please click the FINALIZE QUESTIONNAIRE button above to complete the process and view your summary."), html_content, re.DOTALL)
            if question_match:
                question_text = re.sub(r'<[^>]+>', '', question_match.group(1)).strip()
        
        # Display using the standard method
        if example_text or question_text:
            self._render_example_box(example_text, question_text)
        else:
            # Fallback: display as escaped HTML
            ...
            st.markdown(
                ...
                <div style="display: flex; margin-bottom: 10px;">
                  <div style="background-color: #f0f2f6; border-radius: 15px 15px 15px 0; padding: 10px 15px; max-width: 80%; box-shadow: 1px 1px 3px rgba(0,0,0,0.1);">
                    <p style="margin: 0; color: #333;"><strong>Assistant</strong></p>
                    <div style="margin-top: 5px;">
                      <p style="margin: 0; white-space: pre-wrap;">{escaped_content}</p>
                    </div>
                  </div>
                </div>
                """,
    ...
    def _render_example_box(self, example_text, question_text):
        """Render example and question in properly formatted boxes."""
        html = f"""
        <div style="display: flex; margin-bottom: 15px;">
